...
from core.base.vectorType import (
    TYPE_COO,
    TYPE_COO_SST,
    TYPE_COO_ST,
    TYPE_COO_T,
...
class VectorBase(BaseBase):
    ...
    def toGeoJSONString(self) -> str:
        ...
        for obj in self.data.objects:
            ...
            aFeat["geometry"] = {"type": self.data.type, "coordinates": obj.coordinates}
    ...
    def getInsByOneProp(self, propname: str, value) -> Self:
        ...
        newIns.data.objects = list(
            filter(
                lambda aVec: propname in aVec.properties.keys()
                and aVec.properties.get(propname) == value,
                newIns.data.objects,
            )
        )
        return newIns
    ...
    def getAllAProp(self, propname: str, default="UNDEFINED") -> Sequence[str]:
        return list(
            set(
                list(
                    map(
                        lambda aVec: aVec.properties.get(propname, default),
                        self.data.objects,
                    )
                )
            )
    ...
    def getObjByOneProp(self, propname: str, value) -> AVector | None:
        for aVec in self.data.objects:
            if (
                propname in aVec.properties.keys()
                and aVec.properties.get(propname) == value
            ):
                return aVec
        return None
    ...
    def getObjByOnePropNew(self, propname: str, value) -> AVector | None:
        vecs = list(
            filter(
                lambda aV: propname in aV.properties
                and aV.properties[propname] == value,
                self.data.objects,
            )
        )
        if vecs.__len__() != 0:
    ...
    def getAllObjByOneProp(self, propname: str, value) -> Sequence[AVector]:
        vecs = list(
            filter(
                lambda aV: propname in aV.properties
                and aV.properties[propname] == value,
    ...
    def _transOneCoord(
        self, coo: tuple[float, float], inproj: int, outproj: int
    ) -> tuple[float, float]:
...
if __name__ == "__main__":
    ...
    v.define(
        geojsonVectorIO,
        {"outVectorBase": True, "inFile": True, "inFilePath": "data\\a.geojson"},
        {},
    )
