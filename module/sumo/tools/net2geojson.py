...
def parse_args():
    ...
    argParser.add_argument("--junctions",
                           action="store_true",
                           default=False,
    ...
    argParser.add_argument(
        ...
        action="store_true",
        default=False,
        dest="edgedataTimeline",
        help=
        "exports all time intervals (by default only the first is exported)")
...
def getGeometries(options, net):
    for edge in net.getEdges():
        if options.lanes:
            ...
            yield edge.getID(), edge.getShape(options.junctionCoords), sum(
                [l.getWidth() for l in edge.getLanes()])
...
if __name__ == "__main__":
    ...
    if options.edgeData:
        for i, interval in enumerate(
                sumolib.xml.parse(options.edgeData,
                                  "interval",
                                  heterogeneous=True)):
    ...
    if options.ptlines:
        for ptline in sumolib.xml.parse(options.ptlines,
                                        "ptLine",
                                        heterogeneous=True):
            if ptline.route:
