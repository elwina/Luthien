...
from module.euluc.information import MODULE_ROOT

from loguru import logger


def eulucRun(putout: Callable[[TYPE_Putout], None],
                    instances: MutableMapping[str, TYPE_Instance],
                    optList: Sequence[str]):
    logger.debug("Module euluc Run,optList:{opt}.", opt=",".join(optList))
    ...
    uni = cast(ListConfBase, instances["eulucUni"])
