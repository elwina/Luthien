...
from module.blank.information import MODULE_ROOT

from loguru import logger


def blankRun(putout: Callable[[TYPE_Putout], None],
             instances: MutableMapping[str,
                                       TYPE_Instance], optList: Sequence[str]):
    logger.debug("Module blank Run,optList:{opt}.", opt=",".join(optList))
    ...
    uni = cast(ListConfBase, instances["blankUni"])
